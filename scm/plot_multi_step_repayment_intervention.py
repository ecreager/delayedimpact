...
EXPDIR = '/scratch/gobi1/creager/delayedimpact'
FONTSIZE = 40
LINEWIDTH = 4.0
...
flags.DEFINE_string(
    'baseline_glob',
    os.path.join(EXPDIR, 'multi_step_repayment_intervention.baseline.*'),
    'Glob pattern for all results_dirs in the baseline')
flags.DEFINE_string(
    'policy_intervention_glob',
    os.path.join(
        EXPDIR, 'multi_step_repayment_intervention.policy-intervention.*'
        ), 'Glob pattern for all results_dirs in the policy intervention')
flags.DEFINE_string(
    'repayment_intervention_glob',
    os.path.join(
        EXPDIR, 'multi_step_repayment_intervention.repayment-intervention.*'
...
def main(unused_argv):
    ...
    results_dirs = dict()
    try:
        results_dirs.update(baseline=glob.glob(FLAGS.baseline_glob))
        results_dirs.update(
            policy_intervention=glob.glob(FLAGS.policy_intervention_glob)
        )
        results_dirs.update(
            repayment_intervention=glob.glob(FLAGS.repayment_intervention_glob)
        )
    except:
    ...
    ax.legend(loc='upper right', prop=dict(size=int(FONTSIZE / 2.5)))
    ax.tick_params(axis='both', which='major', labelsize=FONTSIZE // 2)
    ax.tick_params(axis='both', which='minor', labelsize=FONTSIZE // 2)

    filename_a = os.path.join(EXPDIR, 'multi-step-repayment-intervention-a.pdf')
    plt.tight_layout()
    ...
    label_policy_intervention_0 = (
        '$do(f_T \\rightarrow \hat f^{EO}_T), j=$Black'
        )
    ...
    label_policy_intervention_1 = (
        '$do(f_T \\rightarrow \hat f^{EO}_T), j=$White'
        )
    ...
    label_repayment_intervention_0 = (
        '$do(f_Y \\rightarrow \hat f_Y), j=$Black'
        )
    ...
    label_repayment_intervention_1 = (
        '$do(f_Y \\rightarrow \hat f_Y), j=$White'
        )
    ...
    ax.set_ylabel(ylabel, fontsize=FONTSIZE)
    ax.legend(loc='right', prop=dict(size=int(FONTSIZE / 2.5)))
    ax.tick_params(axis='both', which='major', labelsize=FONTSIZE // 2)
    ax.tick_params(axis='both', which='minor', labelsize=FONTSIZE // 2)

    filename_b = os.path.join(EXPDIR, 'multi-step-repayment-intervention-b.pdf')
    plt.tight_layout()
    plt.savefig(filename_b)
    plt.close()

    # TODO(creager): error bars...
